...
describe('ClienteService', () => {
  ...
  const BASE_URL = 'https://apiufpr2021.herokuapp.com/api/v1/clientes/'
  ...
  it('listarTodos() GET - Deve retornar uma lista com 1 cliente', () => {
    ...
    clienteService.listarTodos().subscribe((clientes: Cliente[]) => {
      ...
      expect(clientes[0].cpf).toEqual('03290651621');
      expect(clientes[0].nome).toEqual('João');
      expect(clientes[0].sobreNome).toEqual('José');
  ...
  it('listarTodos() GET - Deve retornar uma lista com n clientes', () => {
    ...
    clienteService.listarTodos().subscribe((cliente: Cliente[]) => {
      ...
      expect(cliente[0].cpf).toEqual("855.594.519-79");
  ...
  it('inserir(cliente: Cliente) POST - Retorna dados iguais ao mock', () => {
    const mockCliente = {
      "cpf": "855.594.519-79",
      "nome": "Fabio",
      "sobreNome": "Marcato",
  ...
  it('atualizar(cliente) PUT - Deve atualizar um cliente', () => {
    ...
    clienteService.atualizar(mockCliente).subscribe((cliente) => {
      expect(cliente.cpf).toEqual("855.594.519-79");
      expect(cliente.nome).toEqual("Fabio2");
      expect(cliente.sobreNome).toEqual("Marcato");
    })
    const req = httpTestingController.expectOne(BASE_URL + mockCliente.id);
    expect(req.request.method).toEqual('PUT');
    req.flush(mockCliente);
  });

  it('remover(cliente.id) DELETE - Deve remover o cliente', () => {
